...
| `qa_crew`里的`CoverageReviewer` | 检查问题覆盖率和证据闭环情况 | 不替代前序 agent 做实质分析 |

### 6.2 标准动作

| 动作 | 谁可以做 | 结果 |
|---|---|---|
| `registry_seed` | `planning_crew` | 创建registry，planner初始化问题思路 |
| `add_question` | `research_crew` `valuation_crew` | 新增一条问题 |
| `add_evidence` |  `research_crew`  | 把证据挂到某个问题下 |
| `status_update` | `research_crew` `qa_crew`| 修改registry的状态 |
| `read_registry` | `research_crew` `valuation_crew` `qa_crew` `diligence_question_designer` | 读取registry中的内容 |
