...
from automated_research_report_generator.tools import AddEntryTool, AddEvidenceTool, ReadRegistryTool, RegistryReviewTool, StatusUpdateTool
...
class OperatingMetricsCrew:
    ...
    pack_title = "运营指标分析包"
    pack_focus = "围绕订单、产能、利用率、客户数和交付验证周期完成运营指标专题研究。"
    output_title = "运营指标分析包"
    search_guidance = "重点查找最能改变投资判断的运营指标；搜不到就明确写缺口，不要编造。"
    extract_guidance = "从 PDF 中补充订单、产能、利用率、交付和客户验证周期等指标线索。"
    qa_guidance = "确认是否已经得到少量但关键的运营指标，而不是泛泛罗列无法支撑判断的数字。"
    synthesize_guidance = "输出要聚焦关键运营指标、同行对比、趋势变化和仍不可确认的缺口。"
    ...
    def qa_check_agent(self) -> Agent:
        ...
        功能：检查当前 operating_metrics pack 的 registry 覆盖度，并补 gap_note 与 next_action。
