...
class PeerInfoCrew:
    ...
    pack_title = "同行信息分析包"
    pack_focus = "围绕同行筛选、可比性约束、peer 财务指标和估值倍数完成同行专题研究。"
    output_title = "同行信息分析包"
    search_guidance = "重点筛选同行名单，并补齐主营差异、区域差异和可比性限制。"
    extract_guidance = "重点从 PDF 中提取管理层自认的可比对象、竞争对手和业务相似度线索。"
    qa_guidance = "确认同行名单、关键 peer 指标和估值倍数是否已经形成可复用底稿。"
    synthesize_guidance = "输出要明确纳入与排除逻辑、peer 财务指标、估值倍数和可比性限制。"
